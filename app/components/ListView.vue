    ...
    <table class="min-w-full divide-y divide-surface-200">
      <thead class="bg-surface-50">
        <tr>
          ...
          <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Pemilik</th>
          <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Tanggal</th>
          <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Ukuran</th>
      ...
      <tbody class="bg-white divide-y divide-surface-200">
        ...
        >
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">{{ folder.userName }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">{{ new Date(folder.createdAt).toLocaleDateString('id-ID') }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">-</td>
          <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
            <button class="text-surface-400 hover:text-surface-600 p-1 rounded-full hover:bg-surface-200 opacity-0 group-hover:opacity-100 transition-opacity" @click.stop="$emit('options', $event, folder.id, 'folder', folder.nameFolder)">
        ...
        >
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
              <div class="flex-shrink-0 h-8 w-8 flex items-center justify-center mr-3 bg-surface-100 rounded-lg text-surface-500">
                ...
                <svg v-else-if="file.fileType.startsWith('image/')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" class="w-5 h-5 fill-current text-purple-400"><path d="M216,40H40A16,16,0,0,0,24,56V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40Zm-4.69,168H44.69l46.2-61.6,23.37,31.16a8,8,0,0,0,12.74.08L166,125.66l45.31,82.34ZM216,195.42,166,104.58a8,8,0,0,0-14-.08l-38.94,51.92-23.4-31.2a8,8,0,0,0-12.71-.09L40,174.58V56H216V195.42ZM96,108a12,12,0,1,1,12-12A12,12,0,0,1,96,108Z"/></svg>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">{{ file.userName }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">{{ new Date(file.createdAt).toLocaleDateString('id-ID') }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-500">{{ driveStore.formatSize(file.fileSize) }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
            <button class="text-surface-400 hover:text-surface-600 p-1 rounded-full hover:bg-surface-200 opacity-0 group-hover:opacity-100 transition-opacity" @click.stop="$emit('options', $event, file.id, 'file', file.nameFile)">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" class="w-5 h-5 fill-current"><path d="M128,96a32,32,0,1,0,32,32A32,32,0,0,0,128,96Zm0,48a16,16,0,1,1,16-16A16,16,0,0,1,128,144Z"/></svg>
            </button>
          </td>
        </tr>
