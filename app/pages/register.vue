<template>
  <div class="auth-page">
    <h2 class="text-2xl font-bold mb-6 text-center text-surface-800">Create your account</h2>
    ...
    <form @submit.prevent="handleRegister" class="space-y-4">
      ...
      <div class="flex flex-col items-center mb-6">
        <div class="relative w-24 h-24 mb-2 group cursor-pointer" @click="triggerFileInput">
          <div class="w-full h-full rounded-full overflow-hidden border-2 border-dashed border-surface-300 flex items-center justify-center bg-surface-50 group-hover:border-primary-500 transition-colors">
            ...
            <span v-else class="text-surface-400 text-xs text-center px-2 group-hover:text-primary-500">Upload Photo</span>
          </div>
      ...
      <div>
        <label for="name" class="block text-sm font-medium text-surface-700 mb-1">Full Name</label>
        <input 
          v-model="name"
          type="text" 
          id="name" 
    ...
    <div class="mt-6 text-center text-sm">
      <p class="text-surface-600">
        ...
        <NuxtLink to="/login" class="font-medium text-primary-600 hover:text-primary-500 transition-colors">Sign in</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
const handleRegister = async () => {
  ...
  const success = await authStore.register(name.value, email.value, password.value, avatarFile.value)
  if (success) {
